...
<section><h2>Breakpoints</h2><form id="breakpoint-form">
<label>Node ID <input id="breakpoint-node" required maxlength="1024"></label>
<label>Kind <select id="breakpoint-kind"><option>entry</option><option>resume</option></select></label>
...
byId('breakpoint-form').onsubmit = event => {
  ...
  try {
    ...
    command({ type: 'setBreakpoint', nodeId: byId('breakpoint-node').value, kind: byId('breakpoint-kind').value,
...
function render(state) {
  ...
  if (state.breakpointHit) byId('status').textContent += ' · ' + state.breakpointHit.kind + ' breakpoint: ' + state.breakpointHit.nodeId + ' [' + state.breakpointHit.activationId + ']' + (state.breakpointHit.handler ? ' → ' + state.breakpointHit.handler : '');
  ...
  for (const breakpoint of state.breakpoints) {
    ...
    item.append(document.createTextNode(breakpoint.nodeId + ' · ' + (breakpoint.kind ?? 'entry') + (breakpoint.inputPath ? ' input ' + JSON.stringify(breakpoint.inputPath) + ' equals ' + JSON.stringify(breakpoint.equals) : ' (unconditional)')));
    const remove = document.createElement('button'); remove.textContent = 'Remove';
    remove.onclick = () => command({ type: 'removeBreakpoint', nodeId: breakpoint.nodeId, kind: breakpoint.kind ?? 'entry' });
